package proj.science.bucket1572.mathematics.geometry
...
class OrthoProjConcept : AbilityConcept(){
    ...
    init {
        ...
        castingTicks = 0
        range = 20.0
        val sunLight: ItemStack = ItemStack(Material.SUNFLOWER, 1)
        sunLight.apply{
            val meta = itemMeta
            meta.setDisplayName("태양 광선")
            meta.addEnchant(Enchantment.CHANNELING, 1, false)
            meta.lore = listOf(
                "태양 광선을 쏘면, 평행한 빛이 입사합니다."
            )
...
class OrthogonalProjection : ActiveAbility<OrthoProjConcept>(){
    ...
    override fun onCast(event: PlayerEvent, action: WandAction, target: Any?) {
        ...
        for (neighbor in neighborhood){
            ...
            var coords : Location = Location(esper.player.world,
                neighbor.location.x,neighbor.location.y - fallDistance, neighbor.location.z) // 낙하시킬 위치
            while (coords.block.isEmpty){
                fallDistance += 1
                coords = Location(esper.player.world, neighbor.location.x,
                    neighbor.location.y - fallDistance, neighbor.location.z)
            } // 가장 가까운 지면 찾기
            fallDistance -= 1 // 한 칸 더 아래를 가리키고 있으므로 조정함.

            neighbor.setLocation(coords)
            if (fallDistance >= concept.minFallDistance){
                val damage : Damage = Damage(DamageType.RANGED,
                EsperStatistic.of(EsperAttribute.ATTACK_DAMAGE to
